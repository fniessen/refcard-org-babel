...
#+Time-stamp: <2014-02-14 Fri 14:19>
#+DESCRIPTION:
#+KEYWORDS:
#+LANGUAGE:  en
...
Above the classical operations of *literate programming*:
...
Org Babel brings /in situ/ code evaluation:

- During *interactive* use
- During *tangle*
- During *export* (code blocks with =:exports= set to =results= or =both=)

This allows you to insert in your document all *data* (that can reasonably be
included) and *code* you used, following the principles of *reproducible research*.
...
2. Default *languages-specific* header arguments shipped with Org mode
   ...
   - Variable =org-babel-default-header-args:ledger= for Ledger
   - Variable =org-babel-default-header-args:org= for Org
   - Variable =org-babel-default-header-args:latex= for LaTeX
   - Variable =org-babel-default-header-args:dot= for Dot
   - Etc.
...
3. *Buffer* (or file) level header arguments
   ...
   For example, when you want to tangle all source blocks in an Org file,
   include the line:

   : #+PROPERTY: tangle yes

   That can be over-ridden on a per-subtree or per-block basis.
...
Currently, a =#+PROPERTY:= line located *anywhere* in a buffer affects the entire
file: it sets a *global* property. In contrast, a property-block is only valid
of the given tree (and subtrees).

In the case of two =#+PROPERTY:= lines for the same property, I would expect the
property to have the later value.

Since *only one property may be specified per property line* there is *no need
for colons*. This mirrors exactly the way the properties are saved under
subtrees in :PROPERTY: blocks.
...
var arguments must be comma-separated. So, the following are now possible:
...
The property mechanism is a general mechanism for the concatenation of
property strings. Any property specification unless it is postfixed with a "+"
will /reset/ the value of that property to its current value.
...
- =:session none= ::
     (default)
     Avoids session-based evaluation.
