...
strings.  Any property specification unless it is postfixed with a ~+~ will
/reset/ the value of that property to its current value.
...
- nothing ::
     If the ~org-confirm-babel-evaluate~ variable is non-nil, allows evaluation
     of the code block (both interactively and during export) without
